...
def snowflakeupload():
    ...
    try:
        ...
        sql_commands = [
            "LIST @my_s3_stage PATTERN='.*metadata\\.json$';",
            "CREATE OR REPLACE TABLE metadata_json (v VARIANT);",
            "COPY INTO metadata_json FROM @my_s3_stage FILE_FORMAT = (TYPE = 'JSON') PATTERN = '.*\\.json$';",
            "SELECT v FROM metadata_json;",
            """
            CREATE OR REPLACE VIEW publications_view AS
            SELECT
                v:document_id::string AS ID,
                v:title::string AS title,
                v:overview::string AS brief_summary,
                CASE 
            ...
            FROM metadata_json;
            """,
            "SELECT * FROM publications_view;",
            "DROP VIEW IF EXISTS metadata_view_s3;",
            "DROP TABLE IF EXISTS PUBLICATIONS_INFO;"
        ]

        print("Number of SQL commands available:", len(sql_commands))

        # Execute SQL commands
        try:
            # Execute the LIST command to check for metadata.json files
            cur.execute(sql_commands[0])
            list_results = cur.fetchall()
            print("Files matching the pattern 'metadata.json' in @my_s3_stage:", len(list_results))
            for file in list_results:
                print(file)

            # Continue executing the remaining SQL commands
            for i, sql in enumerate(sql_commands[1:], start=1):
                cur.execute(sql)
                print(f"SQL command {i} executed successfully.")
            
            # Fetch results from publications_view to verify
            cur.execute(sql_commands[5])
            view_results = cur.fetchall()
            print("Results from SELECT * FROM publications_view:", len(view_results))
